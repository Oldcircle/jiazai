...
import { generateNextSpinner } from './services/geminiService'; // Actually aiService now
import { generateExportPack } from './utils/exportService'; 
import { SpinnerData, CandidateState, ModelConfig, Language } from './types';
...
const PROGENITOR_DATA = (t: any): SpinnerData => ({
  id: 1,
  mutationName: "Material Kinetic",
  reasoning: t.progenitorDesc,
...
export default function App() {
  ...
  const [modelConfigs, setModelConfigs] = useState<ModelConfig[]>(() => {
      const saved = localStorage.getItem('spinner_configs');
      return saved ? JSON.parse(saved) : DEFAULT_CONFIGS;
  });
  const [activeConfigId, setActiveConfigId] = useState<string>(() => {
      return localStorage.getItem('spinner_active_id') || 'default-google';
  });

  const [history, setHistory] = useState<SpinnerData[]>([PROGENITOR_DATA(t)]);
  ...
  // Update Progenitor text if language changes
  useEffect(() => {
      if (!hasStarted && history.length === 1 && history[0].id === 1) {
          setHistory([PROGENITOR_DATA(t)]);
      }
  }, [lang, t, hasStarted, history.length]);

  const saveConfigs = (newConfigs: ModelConfig[], newActiveId: string) => {
      setModelConfigs(newConfigs);
      setActiveConfigId(newActiveId);
      localStorage.setItem('spinner_configs', JSON.stringify(newConfigs));
      localStorage.setItem('spinner_active_id', newActiveId);
  };
  ...
  const getActiveConfig = () => modelConfigs.find(c => c.id === activeConfigId) || modelConfigs[0];
  ...
  const handleEvolve = async (seedOverride?: SpinnerData) => {
    ...
    try {
      ...
      const promiseA = generateNextSpinner(seedCode, nextId, config, (text) => updateCandidateStream('a', text));
      const promiseB = generateNextSpinner(seedCode, nextId, config, (text) => updateCandidateStream('b', text));
      ...
      alert("Generation failed. Check your API Key settings.");
      setCandidates(null);
    } finally {
  ...
  const handleStart = () => {
      ...
      if (!config.apiKey && !hasEnvKey && !config.baseUrl.includes('localhost')) {
          setShowConfig(true);
          return;
      }
      ...
      <div className="flex-none h-14 flex items-center justify-between px-3 md:px-4 bg-black border-b border-neutral-900 z-20">
            <div className="flex items-center gap-2 md:gap-4 flex-1 min-w-0 mr-2">
                 <span className="font-bold tracking-tight text-xs sm:text-sm hidden sm:inline-block">
                     <span className="text-neutral-100">{t.startScreenTitle}</span>
                     <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 via-purple-500 to-blue-400 animate-gradient-text"> {t.startScreenSubtitle}</span>
                 </span>
                 ...
                 <div className="text-[9px] md:text-[10px] text-neutral-500 uppercase tracking-widest font-mono truncate">
                    {!hasStarted ? "" : (isSelectionMode ? `${getActiveConfig().name} / ${getActiveConfig().modelName}` : `${t.history}: ${currentIndex + 1}/${history.length}`)}
                 </div>
            </div>
            ...
            <div className="flex gap-2 flex-none items-center">
                {hasStarted && (
                    ...
                    >
                        {isGenerating 
                            ? t.processing
                            : (isSelectionMode ? <span className="hidden md:inline">{t.selectOnCanvas}</span> : t.evolve)
                        }
                ...
                <button 
                    ...
                    className="h-8 w-8 flex items-center justify-center bg-neutral-900 hover:bg-neutral-800 border border-neutral-800 rounded text-neutral-400 hover:text-white transition-colors ml-2"
                    title={t.settings}
                >
                    ...
                    {candidates?.a.data ? (
                         ...
                         !isGenerating && (
                            <div className="flex flex-col items-center gap-4 opacity-50">
                                <span className="text-xs font-mono text-neutral-700">{t.waitingForStream}</span>
                            </div>
                         )
                    )}
                </div>
            ...
            <div className="absolute inset-0 bg-black/90 backdrop-blur-sm flex flex-col items-center justify-center text-center p-6 z-50 border-t border-neutral-800">
                <div className="animate-in fade-in zoom-in duration-700 slide-in-from-bottom-4 flex flex-col items-center">
                    <h1 className="text-4xl md:text-6xl font-bold text-white mb-4 tracking-tighter max-w-4xl leading-none">
                        {t.startScreenTitle} <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 via-purple-500 to-blue-400 animate-gradient-text">{t.startScreenSubtitle}</span>
                    </h1>
                    
                    <p className="text-sm md:text-base text-neutral-400 max-w-xl mb-8 leading-relaxed font-light">
                        {t.startScreenDesc}
                        <br />
                        <span className="text-white font-medium">{getActiveConfig().modelName}</span> {t.startScreenDesc2.replace('Gemini 3 Flash', '')}
                    </p>
                    ...
                    <div className="mt-8 text-[10px] text-neutral-600 font-mono uppercase tracking-widest hover:text-neutral-500 transition-colors">
                        {t.created_by} <a href="https://x.com/leslienooteboom" target="_blank" rel="noopener noreferrer" className="underline underline-offset-4 decoration-neutral-800 hover:decoration-neutral-500">@leslienooteboom</a>
                    </div>
                </div>
            </div>
        )}
